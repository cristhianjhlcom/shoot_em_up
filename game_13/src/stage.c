...
static void draw_points_pods();
static void draw_debris();
static void draw_explosion();
static void draw_hub();

static void add_points_pod(int x, int y);
...
static void do_points_pods() {
    ...
    prev = &stage.points_head;

    for (e = stage.points_head.next; e != NULL; e = e->next) {
        if (e->x < 0) {
            e->x = 0;
            e->dx = -e->dx;
        }
        ...
        if (player != NULL && collision(
            ...
            e->health = 0;
            stage.score++;
            high_score = MAX(stage.score, high_score);
            play_sound(SND_POINTS, CH_POINTS);
        }

        if (--e->health <= 0) {
            if (e == stage.points_tail) {
                stage.points_tail = prev;
            }
...
static void draw_hub() {
    draw_text(10, 10, 255, 255, 255, "SCORE: %03d", stage.score);

    if (stage.score > 0 && stage.score == high_score) {
        draw_text(960, 10, 0, 255, 0, "HIGH SCORE: %03d", high_score);
    } else {
        draw_text(960, 10, 255, 255, 255, "HIGH SCORE: %03d", high_score);
    }
}
        ...
        if (e->side != b->side && collision(b->x, b->y, b->w, b->h, e->x, e->y, e->w, e->h)) {
            ...
            */
            ...
            if (e == player) {
                ...
                add_points_pod(e->x + e->w / 2, e->y + e->h / 2);
